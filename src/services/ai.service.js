...
class AiService {
  constructor() {
    ...
    this.toolDeclarations = [
      {
        functionDeclarations: [
          ...
          },
          {
            ...
            description: "Get a list of all user bank accounts and their current balances.",
          },
          {
            name: "getGoalsProgress",
            description: "Get the list of user savings goals and the current progress/percentage of each.",
          }
        ]
      }
    ];
    ...
    this.functions = {
      getDashboardSummary: async (userId) => {
        console.log(`[AI-TOOL] Fetching Dashboard Summary for user: ${userId}`);
      ...
      getRecentTransactions: async (userId, args) => {
        ...
        console.log(`[AI-TOOL] Fetching ${limit} Recent Transactions for user: ${userId}`);
      ...
      findTransaction: async (userId, args) => {
        ...
        console.log(`[AI-TOOL] Searching Transaction: "${query}" for user: ${userId}`);
        const transactions = await prisma.transaction.findMany({
          where: { 
            userId,
      ...
      getSpendByCategory: async (userId, args) => {
        ...
        console.log(`[AI-TOOL] Fetching Category Spend (${period}) for user: ${userId}`);
        const now = new Date();
      ...
      getBankAccounts: async (userId) => {
        console.log(`[AI-TOOL] Fetching Bank Accounts for user: ${userId}`);
        const banks = await prisma.bankAccount.findMany({ where: { userId } });
        return banks.map(b => ({
          name: b.name,
          bankName: b.bankName,
          accountType: b.accountType,
          balance: `₹ ${b.balance.toLocaleString('en-IN')}`,
          balanceRaw: b.balance
        }));
      },

      getGoalsProgress: async (userId) => {
        console.log(`[AI-TOOL] Fetching Goal Progress for user: ${userId}`);
        ...
        return goals.map(g => {
          ...
          return {
            ...
            current: `₹ ${g.currentAmount.toLocaleString('en-IN')}`,
            percentage: `${progress.toFixed(1)}%`,
            deadline: g.deadline ? g.deadline.toISOString().split('T')[0] : 'No deadline'
          };
        });
      }
    };
  ...
  async chat(userId, message, history = []) {
    ...
    const systemPrompt = `You are Finza AI, an elite personal Human Financial Advisor for ${userName}. 
    ...
    - ALWAYS use the available tools (getBankAccounts, getGoalsProgress, getRecentTransactions, etc.) to fetch the user's REAL financial data.
    - NEVER GUESSS, invent, or speculate about account balances, transaction amounts, or goal targets. 
    - If you haven't called a tool to see their data yet, you DO NOT KNOW the specific numbers.
    - If a user asks about their goals, you MUST call 'getGoalsProgress' before answering.

    CORE ADVISOR PERSONA:
    - Respond naturally as a knowledgeable, empathetic Human Advisor. 
    - You MUST respond in the SAME LANGUAGE as the user's latest query (Support Hindi, English, Hinglish, Marathi, etc. naturally).
    - Be proactive. If you see their goal is 90% complete, congratulate them! If you see a high expense in "Food", suggest a budget.
    - NEVER use raw text markers like * or _ for lists or emphasis unless it is for **BOLDING**.
    - ALWAYS use **BOLD** (markdown) for amounts, account names, dates, and critical advice. 
    - Formatting example: "Your balance in **HDFC Bank** is **₹ 45,000**."
    - Do NOT use internal codes or technical jargon (like userId or prisma).

    YOUR DATA ACCESS:
    - You have tools to READ the user's Bank Accounts, Transactions, and Goals. 
    - Use these tools to provide factual answers. 
